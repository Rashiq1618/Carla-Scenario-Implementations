...
# UDP Settings
UDP_IP = "127.0.0.1"  # ← Use "127.0.0.1" if you're testing on the same computer
UDP_PORT = 5005
...
def camera_callback(image):
    ...
    camera_surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))  # Adjust for pygame

def spawn_autopilot_vehicles(world, blueprint_library, ego_spawn, count=20, tm_port=8000):
...
def draw_info_panel(surface, ego_vehicle, control, recording):
    panel_width = 200
...
def main():
    ...
    other_vehicles = spawn_autopilot_vehicles(world, blueprint_library, ego_spawn, count=20, tm_port=tm_port)
    ...
    try:
        while True:
            ...
            if obstacle_distance is not None and obstacle_distance < BRAKE_DISTANCE:
            ...
            udp_data = {
                ...
                "obstacle_distance_m": round(obstacle_distance, 2) if obstacle_distance is not None else None
            }
            try:
                udp_socket.sendto(json.dumps(udp_data).encode('utf-8'), (UDP_IP, UDP_PORT))
            except Exception as e:
                print(f"UDP send error: {e}")

            if camera_surface:
                display.blit(camera_surface, (0, 0))
                draw_info_panel(display, ego_vehicle, control, data_logging)
                pygame.display.flip()
